...
const Exporter = {
  ...
  buildExportRows(alignedData) {
    return alignedData.map(row => {
      ...
      State.selectedParams.forEach(p => {
        out[`${p}_sensor`]    = row[`${p}_sensor`];
        out[`${p}_reference`] = row[`${p}_reference`];
        const res = State.analysisResults[p];
        if (res) {
          const { slr, mlr } = res;
          out[`${p}_simple_calibrated`]      = slr.coeff.m * row[`${p}_sensor`] + slr.coeff.b;
          out[`${p}_multilinear_calibrated`] = mlr.coeff.a * row[`${p}_sensor`] + mlr.coeff.b * row.T + mlr.coeff.c * row.RH + mlr.coeff.d;
        }
      });
      out.T  = row.T;
      out.RH = row.RH;
  ...
  toExcel(alignedData) {
    ...
    State.selectedParams.forEach(p => {
      ...
      if (!res) return;
      const { slr, mlr } = res;
      metricRows.push({ Parameter:p, Model:'Simple Linear',
        Equation:`y=${slr.coeff.m.toFixed(6)}x+${slr.coeff.b.toFixed(6)}`,
      ...
      metricRows.push({ Parameter:p, Model:'Multiple Linear',
        Equation:`y=${mlr.coeff.a.toFixed(6)}*sensor+${mlr.coeff.b.toFixed(6)}*T+${mlr.coeff.c.toFixed(6)}*RH+${mlr.coeff.d.toFixed(6)}`,
    ...
    XLSX.writeFile(wb, 'calibration_results.xlsx');
  },
};
...
function renderAnalysis() {
  ...
  params.forEach((p, idx) => {
    ...
    const { slr, mlr } = res;
    const slrEq = `y = ${slr.coeff.m.toFixed(6)}x + ${slr.coeff.b.toFixed(6)}`;
    const mlrEq = `y = ${mlr.coeff.a.toFixed(6)}·sensor + ${mlr.coeff.b.toFixed(6)}·T + ${mlr.coeff.c.toFixed(6)}·RH + ${mlr.coeff.d.toFixed(6)}`;
    const mRow  = (label,v) => `<tr><td>${label}</td><td>${typeof v==='number'?v.toFixed(6):v}</td></tr>`;
    const mTable = (m,label,eq) => `
      <div class="metrics-equation"><strong>${label}:</strong><br><code>${eq}</code></div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('sensorFile').addEventListener('change', async e => {
    ...
    try {
      ...
      UI.pass('Sensor accepted. Upload reference file.');
